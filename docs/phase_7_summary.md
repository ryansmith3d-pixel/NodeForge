...
from `nodeforge.core` and needs a stable, complete interface.
...
from nodeforge.core.executor import execute_graph, register_handler
...
- `src/nodeforge/core/executor.py` — swapped spread order in success return
- `src/nodeforge/core/models.py` — `Node.status` changed to `Literal`, added `Literal` import
- `src/nodeforge/core/__init__.py` — full public API declared
...
NodeForge gets exposed as a standards-compliant agent tool via the Model Context
